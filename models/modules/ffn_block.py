...
class MlpDWBN(nn.Module):
    """
    in_features: input feature_map channel num
    hidden_features: hidden conv layer input channel num
    out_features: output feature_map channel num
    """
    def __init__(
        self,
        in_features,
    ...
    def forward(self, x, H, W):
        if len(x.shape) == 3:
            B, N, C = x.shape
            if N == (H * W + 1):
                # for classification, additional token is added to classify
                cls_tokens = x[:, 0, :]
