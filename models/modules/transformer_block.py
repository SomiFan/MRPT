...
class GeneralTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.attn_type in ["conv"]:
            """modified basic block with seperable 3x3 convolution，分组卷积groups=inplanes时相当于depth-wise conv"""
            ...
            # feed forward net type, conv or identity (perform no computation, as a placeholder)
    ...
    def forward(self, x):
        if self.attn_type in ["conv"]:
            ...
            # Attention, Need H and W to partition window
