...
class LocalPermuteModule(object):
    ...
    def permute(self, x, size):
        n, h, w, c = size
        # h=qh*ph, w=qw*pw, after rearrange, there are qh*qw windows, every window contain ph*pw vectors
        return rearrange(
            x,
...
class MultiheadISAAttention(nn.Module):
    ...
    def forward(self, x, H, W, **kwargs):
        ...
        if self.attn_type in ["isa_local"]:
            # pad, if H，W cannot be division by window size，padding 0 will be needed.
            x_pad = self.pad_helper.pad_if_needed(x, x.size())
            # permute, partition windows, from BxHxWxC to Vec_in_Win x B*Win_Num x C
