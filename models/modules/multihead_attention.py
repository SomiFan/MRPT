...
class MultiheadAttentionRPE(MultiheadAttention):
    ...
    def __init__(self, *args, rpe=False, window_size=7, **kwargs):
        ...
        if rpe:
            ...
            # assume current vector's index is 0, then other vectors' relative index in [-(win_size-1), win_size-1]
    ...
    def forward(
        ...
        # whether to use the same weight matrix to project q,k, and v
        if not self._qkv_same_embed_dim:
            return self.multi_head_attention_forward(
                query,
                key,
    ...
    def multi_head_attention_forward(
        ...
        # bsz: batch_size(window num), tgt_len: target(sequence) length
        tgt_len, bsz, embed_dim = query.size()
        key = query if key is None else key
        value = query if value is None else value
